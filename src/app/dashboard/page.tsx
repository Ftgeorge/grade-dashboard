        ...
        <div className="bg-gray-50 w-full h-screen flex flex-row justify-start items-center">
            <div className="bg-white w-80 h-screen py-6 px-8 flex flex-col gap-5">
                <div className="flex items-center justify-between">
                    <h1 className="text-black text-3xl font-montserrat font-normal"><span className="text-primary">G</span>rade</h1>
                ...
                <div className="py-2 gap-2 flex flex-col">
                    {menuItems.map((item, index) => {
                        ...
                        return (
                            <div key={item.label}>
                                <div
                                    className={`py-2 gap-2 flex items-center justify-start cursor-pointer ${selected === item.label ? 'border border-1 px-2 rounded-lg border-primary bg-primary-main' : 'border border-1 px-2 rounded-lg border-white bg-white'
                                        }`}
                                    onClick={() => handleMenuItemClick(item.label ?? '', item.path ?? '')}
                                >
                                    {React.cloneElement(item.icon!, {
                                        className: `text-lg ${selected === item.label ? 'text-primary' : 'text-gray-500'}`,
                                    })}
                                    <h1
                                        className={`text-sm ${selected === item.label ? 'font-medium text-primary' : ''
                                            }`}
                                    >
                                        {item.label}
                                    </h1>
            ...
            <div className="h-full w-full">
                <div className="w-full h-14 bg-white px-16 rounded-lg flex flex-row items-center justify-between">
                    <PrimaryText placeholder={today} />
                    <div className="flex flex-row h-10 gap-3 justify-center items-center">
                        <LuSearch className="text-xl text-grey" />
                        <input
                            className="rounded-xl px-4 h-10 w-96 bg-white text-grey"
                            placeholder="Search"
                        />
                    </div>
                </div>
